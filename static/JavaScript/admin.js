...
document.addEventListener("DOMContentLoaded", function () {
  // 1. Grab your Django form elements matching the HTML 'name' attributes
  const bookSelect = document.getElementById("bookSelect");
  ...
  if (bookSelect) {
    bookSelect.addEventListener("change", function () {
      ...
      // If user selects "-- Choose a book --", clear the form
      if (!selectedId) {
        titleInput.value = '';
        authorInput.value = '';
      ...
      // (This assumes your views.py is saving 'cachedBooks' as discussed previously)
      const storedBooks = JSON.parse(localStorage.getItem("cachedBooks")) || [];
      const selectedBook = storedBooks.find((b) => String(b.id) === String(selectedId));

      // 4. Fill the form with the selected book's data
      if (selectedBook) {
        titleInput.value = selectedBook.title || ''; 
        authorInput.value = selectedBook.author || '';
        categorySelect.value = selectedBook.category || '';
        
        if(priceInput) {
           priceInput.value = selectedBook.price || '';
        }
        
        // Textareas update via .value in JS
        descriptionTextarea.value = selectedBook.details || selectedBook.description || ''; 
      }
    });
  }
});
